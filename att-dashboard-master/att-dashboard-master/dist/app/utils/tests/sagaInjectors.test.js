...
const sagaInjectors_1 = __importStar(require("../sagaInjectors"));
...
describe('injectors', () => {
    ...
    describe('getInjectors', () => {
        ...
        it('should return injectors', () => {
            expect(sagaInjectors_1.default(store)).toEqual(expect.objectContaining({
        ...
        it('should throw if passed invalid store shape', () => {
            ...
            expect(() => sagaInjectors_1.default(store)).toThrow();
        });
    });
    describe('ejectSaga helper', () => {
        beforeEach(() => {
            store = configureStore_1.default({}, react_router_dom_1.memoryHistory);
            injectSaga = sagaInjectors_1.injectSagaFactory(store, true);
            ejectSaga = sagaInjectors_1.ejectSagaFactory(store, true);
        });
        it('should check a store if the second argument is falsy', () => {
            const eject = sagaInjectors_1.ejectSagaFactory({});
        ...
        it('should cancel a saga in RESTART_ON_REMOUNT mode', () => {
            ...
            store.injectedSagas.test = { task: { cancel }, mode: constants_1.RESTART_ON_REMOUNT };
            ejectSaga('test');
            expect(cancel).toHaveBeenCalled();
        });
        it('should not cancel a daemon saga', () => {
            const cancel = jest.fn();
            store.injectedSagas.test = { task: { cancel }, mode: constants_1.DAEMON };
            ejectSaga('test');
        ...
        it("should remove non daemon saga's descriptor in production", () => {
            ...
            injectSaga('test', { saga: testSaga, mode: constants_1.RESTART_ON_REMOUNT });
            injectSaga('test1', { saga: testSaga, mode: constants_1.ONCE_TILL_UNMOUNT });
            ejectSaga('test');
            ejectSaga('test1');
        ...
        it("should not remove daemon saga's descriptor in production", () => {
            process.env.NODE_ENV = 'production';
            injectSaga('test', { saga: testSaga, mode: constants_1.DAEMON });
            ejectSaga('test');
    ...
    describe('injectSaga helper', () => {
        ...
        it('should check a store if the second argument is falsy', () => {
            const inject = sagaInjectors_1.injectSagaFactory({});
        ...
        it("should validate saga's descriptor", () => {
            ...
            expect(() => injectSaga('test', { saga: testSaga, mode: constants_1.RESTART_ON_REMOUNT })).not.toThrow();
            expect(() => injectSaga('test', { saga: testSaga, mode: constants_1.DAEMON })).not.toThrow();
            expect(() => injectSaga('test', { saga: testSaga, mode: constants_1.ONCE_TILL_UNMOUNT })).not.toThrow();
        });
        ...
        it('should not start daemon and once-till-unmount sagas if were started before', () => {
            store.runSaga = jest.fn();
            injectSaga('test1', { saga: testSaga, mode: constants_1.DAEMON });
            injectSaga('test1', { saga: testSaga, mode: constants_1.DAEMON });
            injectSaga('test2', { saga: testSaga, mode: constants_1.ONCE_TILL_UNMOUNT });
            injectSaga('test2', { saga: testSaga, mode: constants_1.ONCE_TILL_UNMOUNT });
        ...
        it('should start any saga that was not started before', () => {
            ...
            injectSaga('test2', { saga: testSaga, mode: constants_1.DAEMON });
            injectSaga('test3', { saga: testSaga, mode: constants_1.ONCE_TILL_UNMOUNT });
        ...
        it('should not cancel saga if different implementation in production', () => {
            ...
            store.injectedSagas.test = {
                saga: testSaga,
                task: { cancel },
                mode: constants_1.RESTART_ON_REMOUNT,
            };
            function* testSaga1() {
                yield effects_1.put({ type: 'TEST', payload: 'yup' });
            }
            injectSaga('test', { saga: testSaga1, mode: constants_1.DAEMON });
